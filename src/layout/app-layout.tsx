...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiBarChart2 } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<AnalystInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/analysts` : null),
    () => getAnalysts({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Analyst',
      path: isTenantUser ? `/analysts/view/${data?.[0]?.id}` : '/analysts',
      entity: 'analyst',
