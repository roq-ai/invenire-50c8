...
import { getAnalysts, deleteAnalystById } from 'apiSdk/analysts';
import { AnalystInterface } from 'interfaces/analyst';

type ColumnType = ColumnDef<AnalystInterface, unknown>;

interface AnalystListPageProps {
...
export function AnalystListPage(props: AnalystListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AnalystInterface>>(
    () => `/analysts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AnalystInterface) => {
    if (hasAccess('analyst', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/analysts/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <>
          {hasAccess('analyst', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/analysts/edit/${record.id}`} passHref legacyBehavior>
              <Button
          ...
          {hasAccess('analyst', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="analyst" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Analyst
        </Text>
        {hasAccess('analyst', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/analysts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
